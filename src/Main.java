...
public class Main {
    ...
    static File navegarDiretorio(File dir) {
        while (true) {
            ...
            if (index == 0) {
                ...
                return dir;
            } else if (index > 0 && index <= arquivos.length) {
                File escolhido = arquivos[index - 1];
    ...
    static void excluir() {
        ...
        boolean sucesso = excluirRecursivamente(selecionado);
    ...
    static boolean excluirRecursivamente(File arquivo) {
        if (arquivo.isDirectory()) {
            File[] arquivos = arquivo.listFiles();
            if (arquivos != null) {
                for (File f : arquivos) {
                    if (!excluirRecursivamente(f)) return false;
                }
            }
        }
